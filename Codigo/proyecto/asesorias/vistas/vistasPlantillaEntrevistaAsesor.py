...
def obtenerPlantillasDeAsesorCursoAcademico(instancia_asesor_curso_academico):
	try:
		# Obtiene todas las plantillas que pertenecen a un asesor pasado por argumento.
		resultado = models.PlantillaEntrevistaAsesor.objects.filter(dni_pasaporte=instancia_asesor_curso_academico.dni_pasaporte_id, curso_academico=instancia_asesor_curso_academico.curso_academico)
	except:
		resultado = False
	return resultado

# Obtiene una lista ordenada con los ids de las plantillas de un determinado asesor curso academico.
def obtenerListaDeIdsPlantillasDeAsesorCursoAcademico(instancia_asesor_curso_academico):
	if instancia_asesor_curso_academico:
		# Se obtiene una lista con las plantillas de un determinado asesor curso academico.
		lista_plantillas_de_asesor= obtenerPlantillasDeAsesorCursoAcademico(instancia_asesor_curso_academico)
		# Lista que albergara los ids de las asignaturas.
		lista_ids_plantillas_de_asesor = []

		# Si existen plantillas de asesor curso academico extraen sus ids.
		if lista_plantillas_de_asesor:
			# Por cada plantilla de asesor curso academico se extrae su id y se inserta en la nueva lista.
			for plantilla_de_asesor in lista_plantillas_de_asesor:
				lista_ids_plantillas_de_asesor.append(plantilla_de_asesor.id_entrevista_asesor)
			# Ordena la lista con los ids de las plantillas de menor a mayor.
			lista_ids_plantillas_de_asesor.sort()
		# Resultado sera una lista de ids, o una lista vacia si el asesor curso academico no tiene plantillas.
		resultado = lista_ids_plantillas_de_asesor
	# En el caso de que no exista el asesor curso academico se devuelve False.
	else:
		resultado = False
	return resultado

# Determina el primer id_entrevista_asesor disponible para un determinado asesor curso academico.
def determinarSiguienteIdPlantillaDeAsesorCursoAcademico(instancia_asesor_curso_academico):
	# Se obtiene una lista ordenada con los ids de las plantillas existentes para el asesor curso academico.
	lista_ids_plantillas_de_asesor = obtenerListaDeIdsPlantillasDeAsesorCursoAcademico(instancia_asesor_curso_academico)
	...
	while True:
		# La posicion determinada por contador aparece en la lista, por lo tanto se encuentra la id_entrevista_asesor para el asesor curso academico.
		if lista_ids_plantillas_de_asesor.count(contador) > 0:
			contador += 1
		# No existe tal id_entrevista_asesor para el asesor curso academico.
		else:
			break
	return contador
...
def addPlantillaEntrevistaAsesor(request):
	...
	if request.method == 'POST':
		...
		# Se determina el dni_pasaporte y curso academico para ese asesor curso academico.
		dni_pasaporte = instancia_asesor_curso_academico.dni_pasaporte
		curso_academico = instancia_asesor_curso_academico.curso_academico

		# Se determina el siguiente id_entrevista_asesor para el asesor curso academico.
...
def editPlantillaEntrevistaAsesor(request, dni_pasaporte, curso_academico, id_entrevista_asesor):
	...
	if instancia_plantilla_entrevista_asesor:
		...
		form = forms.PlantillaEntrevistaAsesorForm(instance=instancia_plantilla_entrevista_asesor, initial={'asesor_curso_academico': vistasAsesorCursoAcademico.obtenerAsesorCursoAcademico(dni_pasaporte, curso_academico).codigo_asesorCursoAcademico})
	...
	# La plantilla de asesor no existe.
		...
		form = False
	return render_to_response('asesorias/PlantillaEntrevistaAsesor/editPlantillaEntrevistaAsesor.html', {'form': form})
...
def listPlantillaEntrevistaAsesor(request):
	...
	lista_plantillas_entrevista_asesor = models.PlantillaEntrevistaAsesor.objects.all()
	return render_to_response('asesorias/PlantillaEntrevistaAsesor/listPlantillaEntrevistaAsesor.html', {'lista_plantillas_entrevista_asesor': lista_plantillas_entrevista_asesor})
