...
class AlumnoCursoAcademicoFormSelect(forms.Form):
    alumno = forms.ModelChoiceField(queryset=models.AlumnoCursoAcademico.objects.all())

    # Necesario para actualizar el queryset en tiempo de ejecucion, a traves del argumento curso_academico.
    def __init__(self, curso_academico, *args, **kwargs):
        super(AlumnoCursoAcademicoFormSelect, self).__init__(*args, **kwargs)
        self.fields['alumno'].queryset = models.AlumnoCursoAcademico.objects.filter(curso_academico=curso_academico).order_by('dni_pasaporte')
